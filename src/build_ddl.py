...
input_file = 'WaMDaM_DBWrench_Schema_1.01.xml'
use_schemas = True
...
def BUILD_MSSSQL_DDL(opts, ddl_objs, use_schemas):
    ...
    outdir = os.path.join(os.path.abspath(opts.output), 'WaMDaM_1.01_MSSQL.sql')
...
def BUILD_POSTGRES_DDL(opts, ddl_objs, use_schemas):
    ...
    outdir = os.path.join(os.path.abspath(opts.output), 'WaMDaM_1.01_PostgreSQL.sql')
    with open(outdir, 'w') as f:
...
def BUILD_MYSQL_DDL(opts, ddl_objs, use_schemas):
    ...
    outdir = os.path.join(os.path.abspath(opts.output), 'WaMDaM_1.01_MySQL.sql')
    with open(outdir, 'w') as f:
...
def BUILD_SQLITE_DDL(opts, ddl_objs, use_schemas):
    ...
    outdir = os.path.join(os.path.abspath(opts.output), 'WaMDaM_1.01_SQLite.sql')
    with open(outdir, 'w') as f:
...
def main(argv):
    ...
    print '|     WaMDaM DDL Building Tool (Adapted from the ODM Tool)   |'
    print '|---------------------------------|'
